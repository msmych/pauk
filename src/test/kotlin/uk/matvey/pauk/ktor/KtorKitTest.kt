...
class KtorKitTest {
    ...
    @Test
    fun `should respond with CSS`() = testApplication {
        // given
        routing {
            get("/tests/css") {
                call.respondCss {
        ...
        // when
        val rs = client.get("/tests/css")

        // then
        assertThat(rs.status).isEqualTo(OK)
        assertThat(rs.bodyAsText()).isEqualToIgnoringNewLines("body {background-color: #f00;}")
    }
